...
sys.path.append('../')
from datasets_scut import SCUTFBP5500Dataset
...
def load_ecust():
    ...
    train_loader = torch.utils.data.DataLoader(ECUSTDataset(train=True, transform=data_transforms['train']),
                                               batch_size=cfg['batch_size'], shuffle=True, num_workers=50,
                                               pin_memory=True)
    test_loader = torch.utils.data.DataLoader(ECUSTDataset(train=False, transform=data_transforms['test']),
